...
const sql = readFileSync(
  join(__dirname, "..", "supabase", "migrations", "008_resource_grade.sql"),
  "utf8"
);
...
for (const connectionString of candidates) {
  ...
  try {
    ...
    console.log("Migration 008 applied via:", connectionString.replace(encoded, "***"));
    await client.end();
    process.exit(0);
  } catch (error) {
    lastError = error;
...
console.error("Migration 008 failed:", lastError?.message);
process.exit(1);
