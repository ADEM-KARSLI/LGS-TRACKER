...
const sql = readFileSync(
  join(__dirname, "..", "supabase", "migrations", "006_resource_student_source_only.sql"),
  "utf8"
);
...
for (const connectionString of candidates) {
  ...
  try {
    ...
    console.log("Migration 006 applied via:", connectionString.replace(encoded, "***"));
    await client.end();
    process.exit(0);
  } catch (error) {
    lastError = error;
...
console.error("Migration 006 failed:", lastError?.message);
process.exit(1);
