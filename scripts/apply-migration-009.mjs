...
const sql = readFileSync(
  join(
    ...
    "supabase",
    "migrations",
    "009_resource_subject_and_test_page.sql"
  ),
  "utf8"
);
...
for (const connectionString of candidates) {
  ...
  try {
    ...
    console.log("Migration 009 applied via:", connectionString.replace(encoded, "***"));
    await client.end();
    process.exit(0);
  } catch (error) {
    lastError = error;
...
console.error("Migration 009 failed:", lastError?.message);
process.exit(1);
