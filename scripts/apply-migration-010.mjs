...
const sql = readFileSync(
  join(__dirname, "..", "supabase", "migrations", "010_test_templates.sql"),
  "utf8"
);
...
for (const connectionString of candidates) {
  ...
  try {
    ...
    console.log("Migration 010 applied via:", connectionString.replace(encoded, "***"));
    await client.end();
    process.exit(0);
  } catch (error) {
    lastError = error;
...
console.error("Migration 010 failed:", lastError?.message);
process.exit(1);
