...
const sql = readFileSync(
  join(__dirname, "..", "supabase", "migrations", "005_parent_created_students.sql"),
  "utf8"
);
...
for (const connectionString of candidates) {
  ...
  try {
    ...
    console.log("Migration 005 applied via:", connectionString.replace(encoded, "***"));
    await client.end();
    process.exit(0);
  } catch (error) {
    lastError = error;
...
console.error("Migration 005 failed:", lastError?.message);
process.exit(1);
